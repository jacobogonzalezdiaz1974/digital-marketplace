import { factory, isVariableStatement, SyntaxKind, visitEachChild, visitNode, } from "typescript";
 ...
 */
export const specTransformer = (context) => (sourceFile) => {
    let updated = false;
    const visitor = (node) => {
        if (!updated && isVariableStatement(node)) {
            ...
            if (modifiers && modifiers.length === 1 && modifiers[0].kind === EXPORT_KEYWORD) {
                ...
                const newVariableNode = factory.updateVariableDeclaration(variableNode, factory.createIdentifier(SPEC_NAME), variableNode.exclamationToken, variableNode.type, variableNode.initializer);
