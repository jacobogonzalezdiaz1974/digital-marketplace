...
export const copyDirectorySync = (oldPath, newPath) => {
    ...
    for (const dirent of fs.readdirSync(oldPath, { withFileTypes: true })) {
        if (dirent.isDirectory()) {
            ...
            fs.writeFileSync(path.resolve(newPath, dirent.name), fs.readFileSync(path.resolve(oldPath, dirent.name)), { encoding: "utf8" });
        }
    }
};
const visitFileNodes = (source, visitor) => ts.transform(source, [
    (context) => (rootNode) => {
        const visit = (node) => visitor(ts.visitEachChild(node, visit, context));
        return ts.visitNode(rootNode, visit);
    },
]).transformed[0];
const findInFile = (source, predicate) => {
    const matches = [];
    visitFileNodes(source, (node) => {
...
const formatSource = (source) => prettier.format(Array.isArray(source) ? source.join("\n") : source, {
    parser: "typescript",
});
const loadTypescriptModule = async (modulePath) => {
    const jsString = (await build({
...
const loadVersionedSpec = async (specPath, defaultSpec) => {
...
async function addVersionToIndex(sourcePath, version) {
    const source = fs.readFileSync(sourcePath).toString();
    let didUpdate = false;
    const sourceFile = ts.createSourceFile(sourcePath, source, ts.ScriptTarget.ES2015, true);
    const newSource = visitFileNodes(sourceFile, (node) => {
...
const cleanPath = (p) => p.replace(/[^a-zA-Z0-9_\-/.]/, "");
async function initVersionedSpec(specName, options) {
    ...
    fs.writeFileSync(`${rootSpecPath}/index.ts`, await formatSource([
        `import { createVersionedSpec } from "@fig/autocomplete-helpers";\n`,
        `const versionFiles = [];\n`,
        `export default createVersionedSpec("${cleanedSpecName}", versionFiles);\n`,
    ].join("\n")));
}
async function addDiffAction(specName, newSpecPath, diffVersion, options) {
    ...
    const newSpec = (await loadTypescriptModule(newSpecPath)).default;
    ...
    if (lastVersion && semver.compare(diffVersion, lastVersion) <= 0) {
        throw new Error(`Cannot publish version ${diffVersion}, later version ${lastVersion} already exists`);
    }
    const latestSpec = versionNames.map((name) => versions[name]).reduce(applySpecDiff, spec);
    const diff = diffSpecs(latestSpec, newSpec) ?? {};
    const versionExports = findInFile(ts.createSourceFile(currentSpecPath, source, ts.ScriptTarget.ES2015, true), (node) => {
    ...
    fs.writeFileSync(currentSpecPath, await formatSource([
...
async function precomputeAction(files) {
    /*
...
export default new Command("version")
    .addCommand(new Command("add-diff")
    .arguments("<specName> <newSpecFilePath> <diffVersion>")
    .description("generate version diff from new spec and add into old spec")
    .option("-n, --new-path <path>", "The root folder where the updated spec will be saved. Note: this must NOT include the folder spec name")
    ...
    .addCommand(new Command("init-spec")
    .arguments("<name>")
    ...
    .addCommand(new Command("precompute-specs")
    .arguments("<files...>")
