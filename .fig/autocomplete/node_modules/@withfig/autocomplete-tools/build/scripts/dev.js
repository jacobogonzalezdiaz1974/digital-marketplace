...
async function runProgram({ outdir }) {
    console.clear();
    if (os.type() === "Darwin") {
        ...
        if (!fs.existsSync(globalFigAppPath) &&
    ...
    setSetting("autocomplete.devCompletionsFolder", path.join(process.cwd(), outdir ?? "build").replace(/(\s)/g, "\\$1"));
